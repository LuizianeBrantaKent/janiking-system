...
// Debug while wiring (optional; remove in prod)
...
require_once __DIR__.'/../app/TrainingRepository.php';
...
$sortOpt  = $_GET['sort'] ?? 'due_soon'; // due_soon | recent | alpha

/** ---- Pull data ---- */
$kpis         = TrainingRepository::kpis((int)$user['user_id']);
$courses      = TrainingRepository::coursesForUser(
                  (int)$user['user_id'],
                  $q ?: null,
                  $category ?: null,
                  $status ?: null,
...
$current      = TrainingRepository::currentCourse((int)$user['user_id']) ?? ($courses[0] ?? null);
$certificates = TrainingRepository::certificatesForUser((int)$user['user_id']);
        ...
        <div class="card-body">
          <div class="row g-3 align-items-end">
            <div class="col-md-4">
              ...
              <input type="text" name="q" value="<?= e($q) ?>" class="form-control" placeholder="Search courses…" />
            </div>
            <div class="col-md-3">
              ...
              <select name="category" class="form-select form-control">
                ...
                <?php
                  // optional: hard-coded categories to mirror your UI
                  $cats = ['Safety Training','Customer Service','Equipment','HR'];
                  foreach ($cats as $cat) {
                    $sel = $category===$cat ? 'selected' : '';
                    echo "<option $sel>".e($cat)."</option>";
                  }
                ?>
              </select>
            </div>
            <div class="col-md-3">
              ...
              <select name="status" class="form-select form-control">
                ...
                <?php
                  foreach (['Not Started','In Progress','Completed','Overdue'] as $st) {
            ...
            <div class="col-md-2">
              ...
              <select name="sort" class="form-select form-control">
                <option value="due_soon" <?= $sortOpt==='due_soon'?'selected':'' ?>>Due Soon</option>
                <option value="recent"   <?= $sortOpt==='recent'?'selected':'' ?>>Recently Assigned</option>
                <option value="alpha"    <?= $sortOpt==='alpha'?'selected':'' ?>>A–Z</option>
              </select>
            </div>
          </div>
          <div class="mt-3 d-flex gap-2">
            <button class="btnx" type="submit">Apply</button>
            <a class="btnx" href="staff_manage_training.php">Reset</a>
          </div>
        </div>
      </form>
                ...
                <?php foreach ($courses as $c): ?>
                  <tr>
                    ...
                    <td style="min-width:170px">
                      <div class="progress" role="progressbar" aria-valuenow="<?= (int)$c['progress'] ?>" aria-valuemin="0" aria-valuemax="100">
                        <div class="progress-bar" style="width: <?= (int)$c['progress'] ?>%"></div>
                      </div>
                      <div class="text-muted" style="font-size:12px"><?= (int)$c['progress'] ?>% complete</div>
                    </td>
          ...
          <section class="cardx">
            ...
            <div class="card-body">
              <?php if ($current): ?>
                ...
                <div class="mt-2">
                  <div class="progress" role="progressbar" aria-valuenow="<?= (int)$current['progress'] ?>" aria-valuemin="0" aria-valuemax="100">
                    <div class="progress-bar" style="width: <?= (int)$current['progress'] ?>%"></div>
                  </div>
                  <div class="d-flex justify-content-between mt-1">
                    <div class="text-muted" style="font-size:12px"><?= (int)$current['progress'] ?>% complete</div>
              ...
              <?php else: ?>
                <div class="text-muted">No active course.</div>
              <?php endif; ?>
            </div>
          </section>
        </div>
              ...
              <div class="table-wrap">
                <table class="tablex">
                  ...
                  <tbody>
                    ...
                    <?php endforeach; ?>
                    <?php if (!$certificates): ?>
                      <tr><td colspan="4" class="text-muted">No certificates yet.</td></tr>
                    <?php endif; ?>
                  </tbody>
                </table>
              </div>
