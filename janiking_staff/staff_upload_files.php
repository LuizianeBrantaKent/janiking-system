...
// --- JaniKing Upload Files (PHP view) with Bootstrap, icon sidebar, logo-only brand, and heading placed below header ---
...
$recentUploads = [
  ['name' => 'Safety Protocols 2023', 'type' => 'Training',  'category' => 'Safety Training',   'version' => '2.1', 'date' => 'Oct 15, 2023', 'status' => 'Complete'],
  ['name' => 'Equipment Manual - Floor Scrubber', 'type' => 'Document', 'category' => 'Manuals', 'version' => '1.0', 'date' => 'Oct 12, 2023', 'status' => 'Complete'],
  ['name' => 'Customer Interaction Guidelines', 'type' => 'Training', 'category' => 'Customer Service', 'version' => '1.5', 'date' => 'Oct 10, 2023', 'status' => 'Complete'],
  ['name' => 'New Employee Handbook', 'type' => 'Document', 'category' => 'Policies', 'version' => '3.2', 'date' => 'Oct 8, 2023', 'status' => 'Complete'],
  ['name' => 'Chemical Safety Training', 'type' => 'Training', 'category' => 'Safety Training', 'version' => '1.1', 'date' => 'Oct 5, 2023', 'status' => 'Processing'],
];

$currentUpload = [
  'filename' => 'Quarterly Compliance Update.mp4',
  'uploaded' => 45, // MB
  'total'    => 120, // MB
  'percent'  => 38,
  'eta'      => '2 minutes',
...
<body>
  <div class="app">
    ...
    <aside class="sidebar">
      ...
      <nav class="nav">
        ...
        <a href="staff_upload_files.php" class="active">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 2h8l4 4v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2z"/><path d="M14 2v4h4" fill="#fff"/></svg>
          ...
          <section class="cardx">
            ...
            <div class="card-body">
              <div class="mb-2">
                ...
                <input type="text" class="form-control" placeholder="Enter file title" />
              </div>
              <div class="mb-2">
              ...
              <div class="dropzone mb-3" id="dz-training">
                ...
                <button class="btnx" type="button" data-target="#file-training">Browse Files</button>
                <div class="small text-muted">Supported formats: MP4, MOV, AVI (Max size: 500MB)</div>
                <input type="file" id="file-training" hidden />
              </div>

              <button class="btnx btnx-primary" type="button">Upload File</button>
            </div>
          </section>
        </div>
          ...
          <section class="cardx">
            ...
            <div class="card-body">
              ...
              <div class="dropzone mb-3" id="dz-doc">
                <div class="dz-icon">⬆️</div>
                <div>Drag and drop files here or</div>
                <button class="btnx" type="button" data-target="#file-doc">Browse Files</button>
                <div class="small text-muted">Supported formats: PDF, DOCX, XLSX (Max size: 50MB)</div>
                <input type="file" id="file-doc" hidden />
              </div>

              <button class="btnx btnx-primary" type="button">Upload Document</button>
            </div>
          </section>
        </div>
      </div>
  ...
  <script>
    // Lightweight browse + drag helpers (fake only — hook to backend as needed)
    document.querySelectorAll('[data-target]')?.forEach(btn=>{
      btn.addEventListener('click',()=>document.querySelector(btn.getAttribute('data-target')).click());
    });
    const dzones = [document.getElementById('dz-training'), document.getElementById('dz-doc')];
    dzones.forEach(zone=>{
      if(!zone) return;
      ['dragenter','dragover'].forEach(ev=>zone.addEventListener(ev,(e)=>{e.preventDefault(); zone.classList.add('drag');}));
      ['dragleave','drop'].forEach(ev=>zone.addEventListener(ev,(e)=>{e.preventDefault(); zone.classList.remove('drag');}));
      zone.addEventListener('drop',(e)=>{ /* handle files */ });
    });
  </script>
</body>
</html>
